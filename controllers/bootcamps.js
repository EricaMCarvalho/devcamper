...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  const publishedBootcampt = await Bootcamp.findOne({ user: req.user.id });

  // If user is not an admin, they can only add one bootcamp
  if (publishedBootcampt && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
...
// @route   DELETE /api/v1/bootcamps/radious/:zipcode/:distance (could also have /:unit)
// @access  Public
